...
SITELER = []
# Civic.am, Tert.am ve 5tv.am siteleri engellenemez resmi Google News RSS adreslerine yönlendirildi
SITELER.append({"name": "Civic.am", "url": "https://news.google.com/rss/search?q=site:civic.am&hl=hy&gl=AM&ceid=AM:hy", "xml_filename": "civic.xml", "base_url": "https://civic.am", "logo_url": "https://civic.am/assets/img/logo.svg"})
...
SITELER.append({"name": "5tv.am", "url": "https://news.google.com/rss/search?q=site:5tv.am&hl=hy&gl=AM&ceid=AM:hy", "xml_filename": "5tv.xml", "base_url": "https://5tv.am", "logo_url": "https://news.5tv.am//storage/settings/main-logo.png"})
...
SITELER.append({"name": "tert.am", "url": "https://news.google.com/rss/search?q=site:tert.am&hl=hy&gl=AM&ceid=AM:hy", "xml_filename": "tert.xml", "base_url": "https://tert.am", "logo_url": "https://tert.am/resources/favicons/apple-icon-precomposed.png"})
...
for site in SITELER:
    ...
    # --- RADAR.AM İÇİN DOĞRUDAN RESMİ FEED ---
    ...
    # --- GOOGLE NEWS FEED'LERİNİ AYRIŞTIRMA VE TEMİZLEME PROTOKOLÜ (CIVIC, TERT, 5TV) ---
    if site["name"] in ["Civic.am", "tert.am", "5tv.am"]:
        rss_content = fetch_html(site["url"])
        if rss_content and "<rss" in rss_content:
            try:
                # Google News'ten gelen ham XML'i parse et
                root = ET.fromstring(rss_content.encode('utf-8'))
                channel_node = root.find('channel')

                # Bizim temiz RSS yapımızı oluştur
                rss_new = ET.Element("rss", version="2.0")
                channel_new = ET.SubElement(rss_new, "channel")
                ET.SubElement(channel_new, "title").text = site['name']
                ET.SubElement(channel_new, "link").text = site["base_url"]
                ET.SubElement(channel_new, "description").text = f"Cleaned RSS feed from {site['name']} via Google News."
                ET.SubElement(channel_new, "language").text = "hy"
                ET.SubElement(channel_new, "lastBuildDate").text = base_time.strftime("%a, %d %b %Y %H:%M:%S -0000")

                if "logo_url" in site and site["logo_url"]:
                    image_tag = ET.SubElement(channel_new, "image")
                    ET.SubElement(image_tag, "url").text = site["logo_url"]
                    ET.SubElement(image_tag, "title").text = site['name']
                    ET.SubElement(image_tag, "link").text = site["base_url"]
                ...
                for item in channel_node.findall('item'):
                    ...
                    # Google'ın eklediği " - Site İsmi" ibaresini temizle
                    title_text = re.sub(r'\s+-\s+.*$', '', title_text).strip()

                    if len(title_text) < 12: continue

                    new_item = ET.SubElement(channel_new, "item")
                    ET.SubElement(new_item, "title").text = title_text
                    ET.SubElement(new_item, "link").text = link_text
                    ET.SubElement(new_item, "description").text = f"{title_text} - Read on {site['name']}."
                    ET.SubElement(new_item, "guid", isPermaLink="false").text = link_text

                    pub_time = base_time - timedelta(minutes=(count * 2))
                    ET.SubElement(new_item, "pubDate").text = pub_time.strftime("%a, %d %b %Y %H:%M:%S -0000")
                ...
                if count > 0:
                    xml_path = f"NewsFolder/{site['xml_filename']}"
                    if os.path.exists(xml_path): os.remove(xml_path)
                    tree = ET.ElementTree(rss_new)
                    ET.indent(tree, space="  ", level=0)
                    with open(xml_path, "wb") as f:
                        tree.write(f, encoding="utf-8", xml_declaration=True)
                    print(f"Success: {xml_path} generated via Google News proxy.")
                    continue
            except Exception as e:
                print(f"Google News parse error for {site['name']}: {e}")

    # --- DİĞER STANDART SİTELER İÇİN NORMAL KAZIMA AKIŞI ---
    ...
    if "logo_url" in site and site["logo_url"]:
        ...
        ET.SubElement(image_tag, "url").text = site["logo_url"]
        ET.SubElement(image_tag, "title").text = site['name']
        image_tag_link = site["url"].replace(".com", ".news") if site["name"] == "Shamshyan.news" else site["url"]
        ET.SubElement(image_tag, "link").text = image_tag_link
